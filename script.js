...
async function carregarTodosOsDados() {
    ...
    const carregarDoStorage = () => {
        ...
        // CORREÇÃO: Função específica para ler strings sem aspas extras do JSON
        const carregarString = (chave, valorPadrao = '') => {
            const dados = localStorage.getItem(`carteira_${chave}_offline`);
            if (dados) {
                try { return JSON.parse(dados); } catch (e) { return dados; }
            }
            return valorPadrao;
        };
        ...
        // Auto Update: Lê com segurança
        const autoUpdateRaw = localStorage.getItem('carteira_auto_update_enabled_offline');
        try { autoUpdateEnabled = JSON.parse(autoUpdateRaw) === true; } 
        catch (e) { autoUpdateEnabled = autoUpdateRaw === 'true'; }

        dadosSimulacaoNegociar = carregarItem('dados_simulacao_negociar', { fiis: {}, acoes: {}, aporteTotal: '' });
        userName = carregarString('user_name', 'Visitante');
    ...
    if (isLocalStorageEmpty()) {
        console.log("LocalStorage vazio. Tentando carregar dados de demonstração...");
        try {
            ...
            console.log("Dados carregados. Migrando...");
            ...
            // --- CORREÇÃO: Preenche Configurações ---
            urlCotacoesCSV = dadosMigrados.urlCotacoesCSV || dadosMigrados.url_cotacoes_csv || '';
            autoUpdateEnabled = (dadosMigrados.autoUpdateEnabled === true || dadosMigrados.auto_update_enabled === true);
            configuracoesFiscais = dadosMigrados.configFiscais || dadosMigrados.configuracoesFiscais || configuracoesFiscais;
            linksExternos = dadosMigrados.linksExternos || dadosMigrados.links_externos || linksExternos;
            ...
            const dadosParaSalvar = {
                ...
                // --- CORREÇÃO: Salva Configurações Críticas ---
                url_cotacoes_csv: urlCotacoesCSV,
                auto_update_enabled: autoUpdateEnabled,
                configuracoes_fiscais: configuracoesFiscais,
